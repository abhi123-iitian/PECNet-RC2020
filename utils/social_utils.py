...
def collect_data(set_name, dataset_type = 'image', batch_size=36, time_thresh=48, dist_tresh=100, scene=None, verbose=True, root_path="./"):
	...
	rel_path = '/content/drive/My Drive/Copy of PECNet - SR/utils/ETH/{0}/'.format(set_name)
...
class SocialDataset(data.Dataset):
	...
	def __init__(self, set_name="train", b_size=4096, t_tresh=60, d_tresh=50, scene=None, id=True, verbose=True):
		'Initialization'
		load_name = "/content/drive/My Drive/Copy of PECNet - SR/social_pool_data/{0}_{1}{2}_{3}_{4}.pickle".format(set_name, 'all_' if scene is None else scene[:-2] + scene[-1] + '_', b_size, t_tresh, d_tresh)
		print(load_name)
